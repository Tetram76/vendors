...
type
  ...
  private
    ...
    procedure SetGrid(Value: TGzCustomGrid);
  ...
  protected
    ...
    property  Grid: TGzCustomGrid read FGrid;
  public
  ...
  { TGzCustomGrid is an abstract base class that can be used to implement
    ...
    Protected members, for implementors of TGzCustomGrid descendents
      DesignOptionBoost
  ...
  TGzCustomGrid = class(TCustomControl)
  private
    FAnchor: TGridCoord;
...
procedure TInplaceEdit.SetGrid(Value: TGzCustomGrid);
begin
  FGrid := Value;
end;
...
constructor TGzCustomGrid.Create(AOwner: TComponent);
const
...
destructor TGzCustomGrid.Destroy;
begin
  FInplaceEdit.Free;
  inherited Destroy;
  FreeMem(FColWidths);
...
procedure TGzCustomGrid.AdjustSize(Index, Amount: Longint; Rows: Boolean);
var
  NewCur: TGridCoord;
...
function TGzCustomGrid.BoxRect(ALeft, ATop, ARight, ABottom: Longint): TRect;
var
  GridRect: TGridRect;
begin
  GridRect.Left := ALeft;
...
procedure TGzCustomGrid.DoExit;
begin
  inherited DoExit;
  if not (goAlwaysShowEditor in Options) then HideEditor;
end;

function TGzCustomGrid.CellRect(ACol, ARow: Longint): TRect;
begin
  Result := BoxRect(ACol, ARow, ACol, ARow);
end;

function TGzCustomGrid.CanEditAcceptKey(Key: Char): Boolean;
begin
  Result := True;
end;

function TGzCustomGrid.CanGridAcceptKey(Key: Word; Shift: TShiftState): Boolean;
begin
  Result := True;
end;

function TGzCustomGrid.CanEditModify: Boolean;
begin
  Result := FCanEditModify;
end;

function TGzCustomGrid.CanEditShow: Boolean;
begin
...
function TGzCustomGrid.IsActiveControl: Boolean;
var
  H: Hwnd;
...
function TGzCustomGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
end;

function TGzCustomGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
end;

procedure TGzCustomGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
end;

function TGzCustomGrid.GetEditLimit: Integer;
begin
  Result := 0;
end;

function TGzCustomGrid.GetEditStyle(ACol, ARow: Longint): TEditStyle;
begin
  Result := esSimple;
end;

procedure TGzCustomGrid.HideEditor;
begin
  FEditorMode := False;
  HideEdit;
end;

procedure TGzCustomGrid.ShowEditor;
begin
  FEditorMode := True;
  UpdateEdit;
end;

procedure TGzCustomGrid.ShowEditorChar(Ch: Char);
begin
  ShowEditor;
...
procedure TGzCustomGrid.InvalidateEditor;
begin
  FInplaceCol := -1;
  FInplaceRow := -1;
  UpdateEdit;
...
procedure TGzCustomGrid.ReadColWidths(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure TGzCustomGrid.ReadRowHeights(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure TGzCustomGrid.WriteColWidths(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure TGzCustomGrid.WriteRowHeights(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure TGzCustomGrid.DefineProperties(Filer: TFiler);

  function DoColWidths: Boolean;
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(TGzCustomGrid(Filer.Ancestor).FColWidths, FColWidths)
    else
  ...
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(TGzCustomGrid(Filer.Ancestor).FRowHeights, FRowHeights)
    else
...
procedure TGzCustomGrid.MoveColumn(FromIndex, ToIndex: Longint);
var
  Rect: TGridRect;
begin
...
procedure TGzCustomGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
end;

procedure TGzCustomGrid.MoveRow(FromIndex, ToIndex: Longint);
begin
...
procedure TGzCustomGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
end;

function TGzCustomGrid.MouseCoord(X, Y: Integer): TGridCoord;
var
...
procedure TGzCustomGrid.MoveColRow(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
begin
  MoveCurrent(ACol, ARow, MoveAnchor, Show);
end;

function TGzCustomGrid.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := True;
end;

procedure TGzCustomGrid.SizeChanged(OldColCount, OldRowCount: Longint);
begin
end;

function TGzCustomGrid.Sizing(X, Y: Integer): Boolean;
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure TGzCustomGrid.TopLeftChanged;
begin
...
function TGzCustomGrid.CalcCoordFromPoint(X, Y: Integer;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure TGzCustomGrid.CalcDrawInfo(var DrawInfo: TGridDrawInfo);
begin
  CalcDrawInfoXY(DrawInfo, ClientWidth, ClientHeight);
end;

procedure TGzCustomGrid.CalcDrawInfoXY(var DrawInfo: TGridDrawInfo;
  UseWidth, UseHeight: Integer);
...
procedure TGzCustomGrid.CalcFixedInfo(var DrawInfo: TGridDrawInfo);
...
function TGzCustomGrid.CalcMaxTopLeft(const Coord: TGridCoord;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure TGzCustomGrid.CalcSizingState(X, Y: Integer; var State: TGridState;
  var Index: Longint; var SizingPos, SizingOfs: Integer;
...
procedure TGzCustomGrid.ChangeGridOrientation(RightToLeftOrientation: Boolean);
var
  Org: TPoint;
  Ext: TPoint;
begin
...
procedure TGzCustomGrid.ChangeSize(NewColCount, NewRowCount: Longint);
var
...
procedure TGzCustomGrid.ClampInView(const Coord: TGridCoord);
var
  DrawInfo: TGridDrawInfo;
...
procedure TGzCustomGrid.DrawSizingLine(const DrawInfo: TGridDrawInfo);
var
  OldPen: TPen;
begin
  OldPen := TPen.Create;
...
procedure TGzCustomGrid.DrawMove;
var
  OldPen: TPen;
  Pos: Integer;
  R: TRect;
...
procedure TGzCustomGrid.FocusCell(ACol, ARow: Longint; MoveAnchor: Boolean);
begin
...
procedure TGzCustomGrid.GridRectToScreenRect(GridRect: TGridRect;
  var ScreenRect: TRect; IncludeLine: Boolean);
...
procedure TGzCustomGrid.Initialize;
begin
...
procedure TGzCustomGrid.InvalidateCell(ACol, ARow: Longint);
var
  Rect: TGridRect;
begin
  Rect.Top := ARow;
...
procedure TGzCustomGrid.InvalidateCol(ACol: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TGzCustomGrid.InvalidateRow(ARow: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TGzCustomGrid.InvalidateGrid;
begin
  Invalidate;
end;

procedure TGzCustomGrid.InvalidateRect(ARect: TGridRect);
var
  InvalidRect: TRect;
begin
  if not HandleAllocated then Exit;
...
procedure TGzCustomGrid.ModifyScrollBar(ScrollBar, ScrollCode, Pos: Cardinal;
  UseRightToLeft: Boolean);
var
...
procedure TGzCustomGrid.MoveAdjust(var CellPos: Longint; FromIndex, ToIndex: Longint);
var
  Min, Max: Longint;
begin
...
procedure TGzCustomGrid.MoveAnchor(const NewAnchor: TGridCoord);
var
  OldSel: TGridRect;
begin
...
procedure TGzCustomGrid.MoveCurrent(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
var
  OldSel: TGridRect;
...
begin
  ...
  begin
    ...
    // FG pour V&A pour avoir colonne fixe //if Show then ClampInView(FCurrent);
    SelectionMoved(OldSel);
...
procedure TGzCustomGrid.MoveTopLeft(ALeft, ATop: Longint);
var
  OldTopLeft: TGridCoord;
begin
...
procedure TGzCustomGrid.ResizeCol(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure TGzCustomGrid.ResizeRow(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure TGzCustomGrid.SelectionMoved(const OldSel: TGridRect);
var
...
procedure TGzCustomGrid.ScrollDataInfo(DX, DY: Integer;
...
procedure TGzCustomGrid.ScrollData(DX, DY: Integer);
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure TGzCustomGrid.TopLeftMoved(const OldTopLeft: TGridCoord);
...
procedure TGzCustomGrid.UpdateScrollPos;
var
  DrawInfo: TGridDrawInfo;
  MaxTopLeft: TGridCoord;
  GridSpace, ColWidth: Integer;
...
procedure TGzCustomGrid.UpdateScrollRange;
var
...
function TGzCustomGrid.CreateEditor: TInplaceEdit;
begin
  Result := TInplaceEdit.Create(Self);
end;

procedure TGzCustomGrid.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  with Params do
  begin
...
procedure TGzCustomGrid.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TGzCustomGrid.KeyPress(var Key: Char);
begin
...
procedure TGzCustomGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  CellHit: TGridCoord;
  DrawInfo: TGridDrawInfo;
...
begin
  ...
  begin
    ...
    if (CellHit.X >= 0 {FixedCols}) and (CellHit.Y >= FixedRows) then
    begin
...
procedure TGzCustomGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  CellHit: TGridCoord;
begin
...
procedure TGzCustomGrid.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  NewSize: Integer;
...
procedure TGzCustomGrid.MoveAndScroll(Mouse, CellHit: Integer;
...
function TGzCustomGrid.GetColWidths(Index: Longint): Integer;
begin
...
function TGzCustomGrid.GetRowHeights(Index: Longint): Integer;
begin
...
function TGzCustomGrid.GetGridWidth: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TGzCustomGrid.GetGridHeight: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TGzCustomGrid.GetSelection: TGridRect;
begin
  Result := GridRect(FCurrent, FAnchor);
end;

function TGzCustomGrid.GetTabStops(Index: Longint): Boolean;
begin
  if FTabStops = nil then Result := True
  else Result := Boolean(PIntArray(FTabStops)^[Index + 1]);
end;

function TGzCustomGrid.GetVisibleColCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TGzCustomGrid.GetVisibleRowCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure TGzCustomGrid.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TGzCustomGrid.SetCol(Value: Longint);
begin
  if Col <> Value then FocusCell(Value, Row, True);
end;

procedure TGzCustomGrid.SetColCount(Value: Longint);
begin
...
procedure TGzCustomGrid.SetColWidths(Index: Longint; Value: Integer);
begin
  if FColWidths = nil then
    UpdateExtents(FColWidths, ColCount, DefaultColWidth);
...
procedure TGzCustomGrid.SetDefaultColWidth(Value: Integer);
begin
...
procedure TGzCustomGrid.SetDefaultRowHeight(Value: Integer);
begin
...
procedure TGzCustomGrid.SetFixedColor(Value: TColor);
begin
...
procedure TGzCustomGrid.SetFixedCols(Value: Integer);
begin
...
procedure TGzCustomGrid.SetFixedRows(Value: Integer);
begin
...
procedure TGzCustomGrid.SetEditorMode(Value: Boolean);
begin
  if not Value then
    HideEditor
  else
...
procedure TGzCustomGrid.SetGridLineWidth(Value: Integer);
begin
...
procedure TGzCustomGrid.SetLeftCol(Value: Longint);
begin
  if FTopLeft.X <> Value then MoveTopLeft(Value, TopRow);
end;

procedure TGzCustomGrid.SetOptions(Value: TGridOptions);
begin
...
procedure TGzCustomGrid.SetRow(Value: Longint);
begin
  if Row <> Value then FocusCell(Col, Value, True);
end;

procedure TGzCustomGrid.SetRowCount(Value: Longint);
begin
...
procedure TGzCustomGrid.SetRowHeights(Index: Longint; Value: Integer);
begin
  if FRowHeights = nil then
    UpdateExtents(FRowHeights, RowCount, DefaultRowHeight);
...
procedure TGzCustomGrid.SetScrollBars(Value: TScrollStyle);
begin
...
procedure TGzCustomGrid.SetSelection(Value: TGridRect);
var
  OldSel: TGridRect;
begin
  OldSel := Selection;
...
procedure TGzCustomGrid.SetTabStops(Index: Longint; Value: Boolean);
begin
  if FTabStops = nil then
    UpdateExtents(FTabStops, ColCount, Integer(True));
  if Index >= ColCount then InvalidOp(SIndexOutOfRange);
...
procedure TGzCustomGrid.SetTopRow(Value: Longint);
begin
  if FTopLeft.Y <> Value then MoveTopLeft(LeftCol, Value);
end;

procedure TGzCustomGrid.HideEdit;
begin
  if FInplaceEdit <> nil then
    try
      UpdateText;
...
procedure TGzCustomGrid.UpdateEdit;
...
procedure TGzCustomGrid.WMCommand(var Message: TWMCommand);
begin
  with Message do
  begin
...
procedure TGzCustomGrid.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
...
procedure TGzCustomGrid.WMKillFocus(var Msg: TWMKillFocus);
begin
  inherited;
  InvalidateRect(Selection);
...
procedure TGzCustomGrid.WMLButtonDown(var Message: TMessage);
begin
  inherited;
  if FInplaceEdit <> nil then FInplaceEdit.FClickTime := GetMessageTime;
end;

procedure TGzCustomGrid.WMNCHitTest(var Msg: TWMNCHitTest);
begin
  DefaultHandler(Msg);
  FHitTest := ScreenToClient(SmallPointToPoint(Msg.Pos));
end;

procedure TGzCustomGrid.WMSetCursor(var Msg: TWMSetCursor);
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure TGzCustomGrid.WMSetFocus(var Msg: TWMSetFocus);
begin
  inherited;
...
procedure TGzCustomGrid.WMSize(var Msg: TWMSize);
begin
  inherited;
  UpdateScrollRange;
...
procedure TGzCustomGrid.WMVScroll(var Msg: TWMVScroll);
begin
  ModifyScrollBar(SB_VERT, Msg.ScrollCode, Msg.Pos, True);
end;

procedure TGzCustomGrid.WMHScroll(var Msg: TWMHScroll);
begin
  ModifyScrollBar(SB_HORZ, Msg.ScrollCode, Msg.Pos, True);
end;

procedure TGzCustomGrid.CancelMode;
var
  DrawInfo: TGridDrawInfo;
begin
  try
...
procedure TGzCustomGrid.WMCancelMode(var Msg: TWMCancelMode);
begin
  inherited;
  CancelMode;
end;

procedure TGzCustomGrid.CMCancelMode(var Msg: TMessage);
begin
...
procedure TGzCustomGrid.CMFontChanged(var Message: TMessage);
begin
  if FInplaceEdit <> nil then FInplaceEdit.Font := Font;
  inherited;
end;

procedure TGzCustomGrid.CMCtl3DChanged(var Message: TMessage);
begin
  inherited;
  RecreateWnd;
end;

procedure TGzCustomGrid.CMDesignHitTest(var Msg: TCMDesignHitTest);
begin
  Msg.Result := Longint(BOOL(Sizing(Msg.Pos.X, Msg.Pos.Y)));
end;

procedure TGzCustomGrid.CMWantSpecialKey(var Msg: TCMWantSpecialKey);
begin
  inherited;
  if (goEditing in Options) and (Char(Msg.CharCode) = #13) then Msg.Result := 1;
end;

procedure TGzCustomGrid.TimedScroll(Direction: TGridScrollDirection);
var
...
procedure TGzCustomGrid.WMTimer(var Msg: TWMTimer);
var
  Point: TPoint;
  DrawInfo: TGridDrawInfo;
...
procedure TGzCustomGrid.ColWidthsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure TGzCustomGrid.RowHeightsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure TGzCustomGrid.DeleteColumn(ACol: Longint);
begin
  MoveColumn(ACol, ColCount-1);
  ColCount := ColCount - 1;
end;

procedure TGzCustomGrid.DeleteRow(ARow: Longint);
begin
  MoveRow(ARow, RowCount - 1);
  RowCount := RowCount - 1;
end;

procedure TGzCustomGrid.UpdateDesigner;
var
  ParentForm: TCustomForm;
begin
...
function TGzCustomGrid.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function TGzCustomGrid.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function TGzCustomGrid.CheckColumnDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TGzCustomGrid.CheckRowDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TGzCustomGrid.BeginColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TGzCustomGrid.BeginRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TGzCustomGrid.EndColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TGzCustomGrid.EndRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

procedure TGzCustomGrid.CMShowingChanged(var Message: TMessage);
begin
  inherited;
