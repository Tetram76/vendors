...
type
  ...
  protected
    ...
    procedure EncodeUUID(Code: Smallint; Index: Word; const G: TGUID); virtual;
  protected
...
{$ENDIF}
    function GetAsUUID(const Index: Word): TGUID; virtual;
    ...
    procedure SetAsUUID(const Index: Word; const Value: TGUID); virtual;
  ...
  {$ENDIF}
    ...
    function GetByNameAsUUID(const Name: string): TGUID;
    ...
    procedure SetByNameAsUUID(const Name: string; const Value: TGUID);
  public
...
{$ENDIF}
  ...
  {$ENDIF}
    property AsUUID       [const Index: Word]: TGUID      read GetAsUUID       write SetAsUUID;
    ...
    property ByNameAsUUID       [const name: string]: TGUID      read GetByNameAsUUID       write SetByNameAsUUID;
  end;
{ TPoolStream }
...
(******************************************************************************)
  ...
  protected
    ...
    procedure SetAsUUID(const Index: Word; const Value: TGUID); override;
...
//******************************************************************************
  ...
  begin
    ...
    while (CurPos <> nil) do
    begin
      NextPos := {$IFDEF COMPILER18_UP}AnsiStrings.{$ENDIF}StrScan(CurPos, Delimiter);
      if (NextPos = nil) then
        CurStr := CurPos else
        begin
      ...
      begin
        ...
        {$IFDEF COMPILER18_UP}AnsiStrings.{$ENDIF}StrLower(PAnsiChar(CurStr));
        CurStr := Trim(CurStr);
    ...
    begin
      for Result := low(TCharacterSet) to High(TCharacterSet) do
        if (len = Length(CharacterSetStr[Result])) and
          ({$IFDEF COMPILER18_UP}AnsiStrings.{$ENDIF}StrIComp(PAnsiChar(CharacterSetStr[Result]), PAnsiChar(CharacterSet)) = 0) then
            Exit;
...
{$ENDIF}
  ...
  procedure TSQLDA.EncodeUUID(Code: Smallint; Index: Word; const G: TGUID);
  var
    i: Smallint;
    OldLen: SmallInt;
    NewLen: Integer;
...
{$ENDIF}
  ...
  function TSQLDA.GetAsUUID(const Index: Word): TGUID;
  var ASQLCode: SmallInt;
  begin
    CheckRange(Index);
    Result := GuidNull;
...
{$ENDIF}
  ...
  procedure TSQLDA.SetAsUUID(const Index: Word; const Value: TGUID);
  var
    ASQLCode: SmallInt;
  begin
    with FXSQLDA.sqlvar[Index] do
    begin
      ...
      if (sqlscale < 0)  then
        ...
        case ASQLCode of
          SQL_TEXT      : EncodeUUID(SQL_TEXT, Index, Value);
          SQL_VARYING   : EncodeUUID(SQL_VARYING, Index, Value);
{$IFDEF FB25_UP}
...
{$ENDIF}
  ...
  begin
    for i := 0 to GetAllocatedFields - 1 do
      if FXSQLDA.sqlvar[i].AliasNameLength = Length(name) then
        if {$IFDEF COMPILER18_UP}AnsiStrings.{$ENDIF}StrLIComp(PansiChar(@FXSQLDA.sqlvar[i].aliasname), PAnsiChar(Name),
  ...
  function TSQLDA.GetByNameAsUUID(const Name: string): TGUID;
  begin
    result := GetAsUUID(GetFieldIndex(AnsiString(Name)));
  end;
  ...
  procedure TSQLDA.SetByNameAsUUID(const Name: string; const Value: TGUID);
  begin
    SetAsUUID(GetFieldIndex(AnsiString(Name)), Value);
  end;
    ...
    if FXSQLDA.sqln > 1 then
      for i := 0 to FXSQLDA.sqln - 2 do
        if not ((FXSQLDA.sqlvar[i].RelNameLength = FXSQLDA.sqlvar[i+1].RelNameLength) and
          ({$IFDEF COMPILER18_UP}AnsiStrings.{$ENDIF}StrIComp(FXSQLDA.sqlvar[i].RelName, FXSQLDA.sqlvar[i+1].RelName) = 0)) then
            exit;
  ...
  begin
    for Field := 0 to FXSQLDA.sqln - 1 do
      if FXSQLDA.sqlvar[Field].ParamNameLength = Length(name) then
        if {$IFDEF COMPILER18_UP}AnsiStrings.{$ENDIF}StrLIComp(@FXSQLDA.sqlvar[Field].ParamName, PAnsiChar(Name),
...
{$ENDIF}
  ...
  procedure TSQLParams.SetAsUUID(const Index: Word; const Value: TGUID);
  begin
